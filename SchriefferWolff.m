...
compute2SubspaceTranformation::usage =
	"computeSubspaceTranformation[ inputHam, inputOrder, lastLowEnergyIndex, lastMidEnergyIndex] takes in a base Hamiltonian inputHam, and calculates the SW Transformation to the inputOrder-th order, separating the subspace that ends at index lastLowEnergyIndex."

Begin["Private`"]
compute2SubspaceTranformation[inputHam_,inputOrder_,lastLowEnergyIndex_,lastMidEnergyIndex_]:=Module[{HTotMat=inputHam,order=inputOrder,cutoff1=lastLowEnergyIndex,cutoff2=lastMidEnergyIndex,VMat,H0Mat,dim,Hextra,Comm,Sadd,Hextra2,n,m,i,S2,H1,H2,Heff,Stot,A,toReturn,H0,V},(

dim=Dimensions[HTotMat][[1]];
VMat = Table[If[((m <= cutoff1 && n <= cutoff1) || (m > cutoff1 && n > cutoff1)&&((m <= cutoff2 && n <= cutoff2)) || (m > cutoff2 && n > cutoff2)),0, HTotMat[[n, m]]], {n, 1, dim}, {m, 1, dim}];
H0Mat = Table[If[((m <= cutoff1 && n <= cutoff1) || (m > cutoff1 && n > cutoff1)&&((m <= cutoff2 && n <= cutoff2)) || (m > cutoff2 && n > cutoff2)), HTotMat[[n, m]],0], {n, 1, dim}, {m, 1, dim}];
...
Rules = Join[Rules,Array[rule,order]];
Hextra = HextrasSYM[[i]]/.Rules;
SMats[[i]]=Simplify[Table [If[ ((m <= cutoff1 && n <= cutoff1) || (m > cutoff1 && n > cutoff1)&&((m <= cutoff2 && n <= cutoff2)) || (m > cutoff2 && n > cutoff2)),0,-((I Hextra[[n,m]])/(H0Mat[[n]][[n]]\[Minus]H0Mat[[m]][[m]]))],{n,1,dim},{m,1,dim}]];
Stot = Stot + (A^i)*SMats[[i]];
];
toReturn = IdentityMatrix[dim];
Do[toReturn = toReturn+MatrixPower[-I*Stot,i]/i!,{i,1,order,1}];
